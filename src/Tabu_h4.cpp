...
void busqueda_tabu(vector<vector<int>>& viajes, vector<vector<int>>& rivales, double& distancia, int max_iter, int max_tabu_size, vector<int>& minimo_dia, vector<int>& maximo_dia, vector<vector<int>>& partidos_dia) {
    double distancia_mejor = distancia;
    vector<vector<int>> mejor_calendario;
    mejor_calendario = copiar_calendario(viajes);

    set<tuple<int, int, int, int>> lista_tabu;

    for (int iter = 0; iter < max_iter; iter++) {
        //cout << iter << endl;
        double mejor_dist_vec = numeric_limits<double>::max();
        tuple<int, int, int,int> mejor_mov;
        vector<vector<int>> mejor_vecino = viajes;
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (viajes[i][k] != -1) {                           //si juega partido en esa jornada
                    ...
                    for (int k2 = 0; k2 < viajes[0].size(); k2++) {     
                        if (k2 != k) {
                            if (rivales[local][k2] == -1 && rivales[visitante][k2] == -1 && !lista_tabu.count({ k, k2, local, visitante })) {
                                ...
                                if (dist < 0) {
                                    ...
                                    if (!tieneDescansoExcesivo(viajes, local) && !tieneDescansoExcesivo(viajes, visitante) && !tieneTresPartidosSeguidos(viajes, local) && !tieneTresPartidosSeguidos(viajes, visitante) && comprueba_balance(minimo_dia, maximo_dia) && comprueba_balance_lyv(viajes, local) && comprueba_balance_lyv(viajes, visitante) && dist < mejor_dist_vec) {
                                        ...
                                        mejor_mov = {k,k2, local, visitante };
...
int main() {
    ...
    // Inicialización de estructuras para el conteo de partidos
    ...
    ofstream archivo2("resultados_tabu_h4.txt"); // Abre el archivo en modo lectura
    ...
    vector<int> m_iter = { 500 };
    vector<int> t_size = { 10, 50};

    for (int i : m_iter) {
        for (int s : t_size) {
            busqueda_tabu(viajes, rivales, distanciaTotal, i, s, minimo_dia, maximo_dia, partidos_dia);
            archivo2 << "Max iter = " << i << ", Tabu size = " << s << ", Distancia -> " << distanciaTotal << endl;
            cout << "Max iter = " << i << ", Tabu size = " << s << ", Distancia -> " << distanciaTotal << endl;
    ...
    imprimeCalendario(mejor_calend, "Tabu_h4.txt");
